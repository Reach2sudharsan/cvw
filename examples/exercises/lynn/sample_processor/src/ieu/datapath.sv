...
module datapath(
    ...
    // flopenr #(1) F2D_Funct7b5(.clk(clk), .reset(reset), .enable(1'b1), .flush(1'b0), .D(Funct7b5F), .Q(Funct7b5D));
    // flopenr #(7) F2D_Op(.clk(clk), .reset(reset), .enable(1'b1), .flush(1'b0), .D(OpF), .Q(OpD));

    // flopenr #(1) F2D_IsJalr(.clk(clk), .reset(reset), .enable(1'b1), .flush(1'b0), .D(Funct7b5F), .Q(Funct7b5D));
    ...
    // Execute to Memory Controller registers
    ...
    // flopenr #(32) E2M_IEUAdr(.clk(clk), .reset(reset), .enable(1'b1), .flush(1'b0), .D(IEUAdrE), .Q(IEUAdrM));
    // flopenr #(32) E2M_RD2(.clk(clk), .reset(reset), .enable(1'b1), .flush(1'b0), .D(RD2E), .Q(RD2M));


    // Execute to Memory pipeline for Funct3 (used by StoreTypeM/LoadTypeM)
    flopenr #(3) E2M_Funct3(.clk(clk), .reset(reset), .enable(1'b1), .flush(1'b0), .D(Funct3E), .Q(Funct3M));
    ...
    assign LoadTypeM = (OpM == 7'b0000011) ? Funct3M : 3'b111; // determines type of load instruction to perform
